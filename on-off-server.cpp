...
EmberAfStatus OnOffServer::setOnOffValue(chip::EndpointId endpoint, chip::CommandId command, bool initiatedByLevelChange)
{
    ...
    if((currentValue && command) == Commands::On::Id){
        xEventGroupSetBits(APP_event_group,APP_event_button_toggle_BIT);
    }
    ...
    {
        emberAfOnOffClusterPrintln("On/off already set to new value:%d",currentValue && command);
