    ...
    while(1)
    {
        ...
        if(data_len > 0){
            ...
            for(i = 0;i < 80;i++){
                ...
                //luminance:9 12
                if(('l' == data[i]) && ('u' == data[i+1]) && ('m' == data[i+2]) && ('i' == data[i+3])   \
                    && ('n' == data[i+4]) && ('a' == data[i+5]) && ('n' == data[i+6]) && ('c' == data[i+7]) \
                    && ('e' == data[i+8]) && (':' == data[i+9])){
                    first_bit = i;
                    data_ok = 12;
                    break;
                ...
                //restart:7 13
                if(('r' == data[i]) && ('e' == data[i+1]) && ('s' == data[i+2]) && ('t' == data[i+3])   \
                    && ('a' == data[i+4]) && ('r' == data[i+5]) && ('t' == data[i+6]) && (':' == data[i+7])){
                    first_bit = i;
                    data_ok = 13;
                    break;
                }
                ...
                //reset:5 14
                if(('r' == data[i]) && ('e' == data[i+1]) && ('s' == data[i+2]) && ('e' == data[i+3])   \
                    && ('t' == data[i+4]) && (':' == data[i+5])){
                    first_bit = i;
                    data_ok = 14;
                    break;
...
/*
                ...
                */
            ...
            else if(data_ok == 12){//luminance:9 12
                first_len = data_len - (first_bit + 10);
                luminance = 0;
                for(i = 0;i < first_len;i++){
                    if((data[first_bit + i + 10] >= '0') && (data[first_bit + i + 10] <= '9')){
                        luminance = 10 * luminance + (data[first_bit + i + 10] - '0');
                    }
                }
                ...
                for(i = 0;i < first_len;i++){
                    if((data[first_bit + i + 8] >= '0') && (data[first_bit + i + 8] <= '9')){
                        temp = 10 * temp + (data[first_bit + i + 8] - '0');
                    }
                }
                ESP_LOGI("htmltomcu","restart:%d",temp);
                if(temp >= 170){
                    esp_restart();
                }
            }
            else if(data_ok == 14){//reset:5 14
                first_len = data_len - (first_bit + 6);
                temp = 0;
                for(i = 0;i < first_len;i++){
                    if((data[first_bit + i + 6] >= '0') && (data[first_bit + i + 6] <= '9')){
                        temp = 10 * temp + (data[first_bit + i + 6] - '0');
                    }
                }
                ESP_LOGI("htmltomcu","reset:%d",temp);
                if(temp >= 170){
                    reset = 170;
                }
            }    
